...
class QuantizableMobileNetV3_Household_qat(nn.Module):
...
def train_model_qat(
    ...
    start_qat = training_config.get('start_qat', True)
    ...
    print(f"Freeze BN stats epochs: {freeze_bn_epochs}")
    print(f"QAT will be activated dynamically within script")
    ...
    start_qat = True #QAT starts from epoch 0
    if start_qat:
        start_qat_epoch = 0
        model = _prepare_qat_model(model, backend)
        start_qat_save = True
    else:
        start_qat_epoch = 1000 #set it to high value
        start_qat_save = False
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            if epoch >= start_qat_epoch + 4:
                print('Disabling observer for subseq epochs, epoch = ', epoch)
                model.apply(torch.quantization.disable_observer)
            if epoch >= start_qat_epoch + freeze_bn_epochs:
        ...
        if start_qat: 
            ...
            quantized_model = torch.quantization.convert(eval_model, inplace=False)
            ...
            #if test_accuracy > 88.0 set flag start_qat to True and call prapare qat on model and set optimizer
            #with prepared model
            if test_accuracy > 88.0:
                start_qat = True
                start_qat_epoch = epoch + 1
                model = _prepare_qat_model(model, backend)
                print("Start debug QAT start......")
                model.to('cuda')
                print("cuda debug QAT start......")
                lr=0.01
                ...
                print("Start debug QAT 234 start...... ")
                print("#####################################################")
                print(f'Starting QAT training now. Epoch number: {epoch} LR: {lr}')
                print("#####################################################")
        ...
        if epoch >= start_qat_epoch:
            if scheduler is not None:
                if isinstance(scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                    scheduler.step(test_accuracy) #test_loss)
                    print("plateau scheduler stepped")
                else:
                    scheduler.step()
        ...
        # Save best model
        if test_accuracy > best_accuracy and start_qat_save: #epoch >= qat_start_epoch:
