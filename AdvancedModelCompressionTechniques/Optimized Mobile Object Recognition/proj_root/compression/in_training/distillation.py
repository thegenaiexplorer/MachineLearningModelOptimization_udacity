...
class MobileNetV3_Household_Small(nn.Module):
    ...
    def __init__(self, num_classes=10, width_mult=1, linear_size=256, dropout=0.2, weights="DEFAULT"):
        super().__init__()
        self.width_mult=width_mult
        self.linear_size=linear_size
        self.dropout=dropout
        self.weights = weights
        self.model = models.mobilenet_v3_small(weights=self.weights, width_mult=self.width_mult)
        lastconv_output_channels = self.model.classifier[0].in_features
        self.model.classifier = nn.Sequential(
            nn.Linear(lastconv_output_channels, self.linear_size),
            nn.Hardswish(inplace=True),
            nn.Dropout(p=self.dropout, inplace=True),
            nn.Linear(self.linear_size, num_classes),
            )
...
def _knowledge_distillation_loss(criterion, student_logits, teacher_logits, targets, temperature=2.0, alpha=0.5):
    """
...
def train_with_distillation(
    ...
    student_model = load_model(checkpoint_path, device, model_class=MobileNetV3_Household_Small, width_mult=student_model.width_mult, linear_size=student_model.linear_size, dropout=student_model.dropout, weights=student_model.weights)
