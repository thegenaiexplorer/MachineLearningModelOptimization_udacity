...
def compare_pipeline_experiments(
    experiments=None,
    compression_results=None,
    baseline_metrics=None,
    baseline_name=None,
    ...
    if experiments is not None:
        compression_results = {
            exp_name: load_metrics(os.path.join(results_dir, exp_name, "pipeline_metrics.json"))
            for exp_name in experiments
        }
        compression_results = {k: v for k, v in compression_results.items() if v is not None}
    ...
    # Process results
    results_table = []
    eval_base_req_met = False
    base_size_reduction = 0
    base_inference_speedup = 0
    base_accuracy_change = 0

    results_table.append({
            'Technique': "baseline_mobilenet",
            'Model Size (MB)': f"{baseline_size:.2f}",
            'Size Reduction': f"{base_size_reduction*100:.1f}%",
            'Inference Time (ms)': f"{baseline_time:.2f}",
            'Speedup': f"{base_inference_speedup:.1f}x",
            'Accuracy (%)': f"{baseline_acc:.2f}",
            'Acc. Change': f"{base_accuracy_change*100:+.1f}%",
            'All Reqs Met': "✓" if eval_base_req_met else "✗"
        })
    ...
    for key, result in compression_results.items():
        ...
        # Extract metrics
        model_size_mb = result['final_metrics'].get('size', {}).get('model_size_mb', 0)
        total_params = result['final_metrics'].get('size', {}).get('total_params', 0)
        accuracy = result['final_metrics'].get('accuracy', {}).get('top1_acc', 0)
        inference_time_ms = result['final_metrics'].get('timing', {}).get(device_priority, {}).get('avg_time_ms', 0)
