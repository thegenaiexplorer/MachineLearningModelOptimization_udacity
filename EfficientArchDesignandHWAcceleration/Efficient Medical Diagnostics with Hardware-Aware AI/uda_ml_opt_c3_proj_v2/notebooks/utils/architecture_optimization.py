...
def create_optimized_model(base_model: nn.Module, optimizations: Dict[str, Any]) -> nn.Module:
    ...
    for k, v in optimizations.items():
        if (isinstance(v, bool) and v):
            optimization_order.append(k)
    print(f'Requested Optimization Order is {optimization_order}')
...
def apply_interpolation_removal_optimization(model, native_size: int = 64) -> nn.Module:
    """
    ...
    class ResNet18(nn.Module):
        def __init__(self):
            ...
            self.residual_block_1_0 = nn.Sequential(
            ...
            self.residual_block_1_1 = nn.Sequential(
                nn.Conv2d(64, 64, 3, padding=1, bias=False),
                nn.BatchNorm2d(64),
                nn.ReLU(inplace=True),
                nn.Conv2d(64,64,3,padding=1, bias=False),
...
def apply_depthwise_separable_optimization(
    ...
    replacements = len(model_name_module_tup_list)
    ...
    class ResNet18(nn.Module):
        def __init__(self):
            ...
            self.residual_block_2_0 = nn.Sequential(
                nn.Conv2d(64, 64, 3, padding=1, stride=2, groups=64, bias=False),
                nn.BatchNorm2d(64),
                nn.ReLU(inplace=True),
            ...
            self.residual_block_3_0 = nn.Sequential(
                nn.Conv2d(128, 128, 3, padding=1, stride=2, groups=128, bias=False),
                nn.BatchNorm2d(128),
                nn.ReLU(inplace=True),
...
def apply_inverted_residual_optimization(
    ...
    class ResNet18(nn.Module):
        def __init__(self):
            ...
            self.residual_block_1_0 = nn.Sequential(
                ...
                nn.BatchNorm2d(64*3),
                nn.ReLU(inplace=True),
                nn.Conv2d(64*3, 64*3, 3, padding=1, groups=64*3, bias=False),
                nn.BatchNorm2d(64*3),
                nn.ReLU(inplace=True),
            ...
            self.residual_block_2_0 = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Conv2d(128*2, 128*2, 3, padding=1, stride=2, groups=128*2, bias=False),
                nn.BatchNorm2d(128*2),
                nn.ReLU(inplace=True),
                nn.Conv2d(128*2, 128, 1, bias=False),
                nn.BatchNorm2d(128),
                )
            self.residual_block_2_1 = nn.Sequential(
                ...
                nn.BatchNorm2d(128*2),
                nn.ReLU(inplace=True),
                nn.Conv2d(128*2, 128*2, 3, padding=1, groups=128*2, bias=False),
                nn.BatchNorm2d(128*2),
                nn.ReLU(inplace=True),
                nn.Conv2d(128*2, 128, 1, bias=False),
                nn.BatchNorm2d(128),
            ...
            self.residual_block_3_0 = nn.Sequential(
                nn.Conv2d(128, 256*2, 1, bias=False),
                nn.BatchNorm2d(256*2),
                nn.ReLU(inplace=True), 
                nn.Conv2d(256*2, 256*2, 3, padding=1, stride=2, groups=256*2, bias=False),
                nn.BatchNorm2d(256*2),
                nn.ReLU(inplace=True),
            ...
            self.residual_block_3_1 = nn.Sequential(
                ...
                nn.BatchNorm2d(256*2),
                nn.ReLU(inplace=True), 
                nn.Conv2d(256*2, 256*2,3, padding=1, groups=256*2, bias=False),
                nn.BatchNorm2d(256*2),
                nn.ReLU(inplace=True),
                nn.Conv2d(256*2, 256,1, bias=False),
                nn.BatchNorm2d(256),
            ...
            self.residual_block_4_0 = nn.Sequential(
                nn.Conv2d(256, 512*2, 1, bias=False),
                nn.BatchNorm2d(512*2),
                nn.ReLU(inplace=True),
                nn.Conv2d(512*2, 512*2, 3, padding=1, stride=2, groups=512*2, bias=False),
                nn.BatchNorm2d(512*2),
                nn.ReLU(inplace=True),
                nn.Conv2d(512*2, 512, 1, bias=False),
                nn.BatchNorm2d(512),
                )
            self.residual_block_4_1 = nn.Sequential(
                nn.Conv2d(512, 512*2,1, bias=False),
                nn.BatchNorm2d(512*2),
                nn.Conv2d(512*2, 512*2,3, padding=1, groups=512*2, bias=False),
                nn.BatchNorm2d(512*2),
                nn.ReLU(inplace=True),
                nn.Conv2d(512*2, 512,1, bias=False),
                nn.BatchNorm2d(512),
    ...
    optimized_model = ResNet18()
    total_candidates = len(mod_list)
    replacements = total_candidates #len([j for l in list(mod_list.values()) for j in l])
    if replacements > 0:
